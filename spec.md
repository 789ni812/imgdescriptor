...
- **Data Model:**  
  ...
  - **All choices/options after each story must be generated by the LLM, tailored to the story and character.**
  - **All generated choices and outcomes must be saved to the template (per turn) for replay, export, and sharing.**
- **UI/UX:**  
  - After each story, present 2–3 LLM-generated choices to the player.
  - Show outcome and update stats/inventory accordingly.
- **API/LLM:**  
  - Update prompts to ask the LLM to suggest choices based on the story and character state.
- **TDD Steps:**  
  1. Write failing test for rendering LLM-generated choices after a turn.
  2. Implement UI and state logic.
  3. Test LLM prompt for generating choices.
  4. Refactor and ensure all tests pass.
- **Template Integration:**  
  - **All generated choices and outcomes must be saved to the template (per turn).**
  - Allow templates of type `game` to define custom choice logic or use LLM-generated choices.
  - **The final story generation must reference the choices made and their outcomes.**
  - **This is required for generation type 'game', and optional/disabled for other types.**
- **Note:**  
  - This makes the game more dynamic, replayable, and extensible for future use cases.

            ...
            {isDescriptionLoading ? (
                ...
                <div className="space-y-4">
                  {imageDescription.setting && (
                    <div>
                      <h3 className="font-semibold text-base mb-1">Setting</h3>
                      <p className="text-card-foreground whitespace-pre-line">{imageDescription.setting}</p>
                    </div>
                  )}
                  {imageDescription.objects && imageDescription.objects.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-base mb-1">Objects</h3>
                      <ul className="list-disc ml-6 text-card-foreground whitespace-pre-line">
                        {imageDescription.objects.map((obj, i) => obj && <li key={i}>{obj}</li>)}
                      </ul>
                    </div>
                  )}
                  {imageDescription.characters && imageDescription.characters.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-base mb-1">Characters</h3>
                      <ul className="list-disc ml-6 text-card-foreground whitespace-pre-line">
                        {imageDescription.characters.map((char, i) => char && <li key={i}>{char}</li>)}
                      </ul>
                    </div>
                  )}
                  {imageDescription.mood && (
                    <div>
                      <h3 className="font-semibold text-base mb-1">Mood & Atmosphere</h3>
                      <p className="text-card-foreground whitespace-pre-line">{imageDescription.mood}</p>
                    </div>
                  )}
                  {imageDescription.hooks && imageDescription.hooks.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-base mb-1">Narrative Hooks</h3>
                      <ul className="list-disc ml-6 text-card-foreground whitespace-pre-line">
                        {imageDescription.hooks.map((hook, i) => hook && <li key={i}>{hook}</li>)}
                      </ul>
                    </div>
                  )}
                </div>

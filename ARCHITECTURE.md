...
## Arena Generation (2024 Update)

### Key Features
- **Filename-based Naming:** Arena names are now derived from the uploaded image filename whenever possible (e.g., `tokyo-arena.jpg` → "The Tokyo Arena").
- **Combat-Focused Descriptions:** Arena descriptions are generated by an LLM with a strict prompt to keep them under 60 words, focusing on tactical, fighting-game language and omitting atmospheric prose.
- **LLM Prompt/Response Handling:** The backend expects a plain text response from the LLM and post-processes it for brevity and clarity.
- **Fallbacks:** If the filename is not meaningful, the system falls back to description-based naming using keywords in the image analysis or description.

### Arena Generation Flow
1. **Image Upload:** User uploads an arena image.
2. **Filename Extraction:** The backend extracts the filename and attempts to generate a dramatic arena name from it.
3. **LLM Description Generation:** The backend sends a prompt to the LLM, requesting a punchy, tactical description (max 60 words, no JSON, no formatting).
4. **Post-processing:** The backend cleans the LLM response, ensuring it is concise and action-oriented.
5. **Metadata Storage:** The generated name, description, and environmental highlights are saved as arena metadata for future use.

### Example
- **Filename:** `tokyo-arena.jpg`
- **Generated Name:** `The Tokyo Arena`
- **Generated Description:** `Neon rain slicks the chrome of the Tokyo Arena. Fighters dodge laser grids, use billboards as cover, and exploit magnetic fields for tactical advantage. Every shadow is a threat; survival demands ruthless precision.`

### Rationale
- **Immersion:** Naming arenas after their visual theme or filename increases immersion and variety.
- **Gameplay Focus:** Short, punchy descriptions help players quickly understand tactical opportunities.
- **LLM Robustness:** By handling plain text and enforcing brevity, the system is resilient to LLM verbosity or formatting quirks.
